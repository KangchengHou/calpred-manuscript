...
def fit_het_linear2(
    ...
    if slope_covar is None:
        fit = fit_het_linear.r_ext.fit_het_linear2(  # type: ignore
            y=y.reshape(-1, 1), mean_covar=mean_covar, var_covar=var_covar, trace=trace
        )
...
def calibrate_and_adjust2(
    ...
    """
    Perform the calibration and adjust
    DO NOT include the intercept in all covariates
    ...
    if fit_slope:
        mean_beta, var_beta, intercept_beta, slope_beta = fit
        pred_mean = (
            test_mean_covar.dot(mean_beta)
            * (1 + test_slope_covar.dot(slope_beta))  # type: ignore
            + intercept_beta
        )
        pred_std = np.sqrt(np.exp(sm.add_constant(test_var_covar).dot(var_beta)))
        return pred_mean, pred_std, mean_beta, var_beta, intercept_beta, slope_beta
    else:
        mean_beta, var_beta = fit
        pred_mean = sm.add_constant(test_mean_covar).dot(mean_beta)
        pred_std = np.sqrt(np.exp(sm.add_constant(test_var_covar).dot(var_beta)))
        return pred_mean, pred_std, mean_beta, var_beta